...
> **注意（位置づけ）**: 密なメッシュのモデルでは、これはモノリシック解法より**速くなりません**
> （一様ダウンサンプリングの方が生 raw 速度は上）。DD の利点は「ギャップ／飽和部を完全に解像
...
> **⚠️ 「全バンド粗大化」は一様ダウンサンプリングとは別物**: 例えば全帯を `cf=2` にしても、それは
> 「1枚の粗グリッドを自己無撞着に解く一様ダウンサンプリング」**ではなく**、粗いサブドメインを Robin
> で結合した別アルゴリズムで、アクティブ帯を粗くしているため**不安定（発散）**です。一様に解像度を
> 落としたいだけなら DD ではなく画像そのものを縮小してください。
