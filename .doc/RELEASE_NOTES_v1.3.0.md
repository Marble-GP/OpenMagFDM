...
**レアユニーク色検出**（`afced29`）:
レア色（占有率 ≤ rareThreshold）のうち、いずれのドミナント色ペアの AA ブレンドとしても説明できない色を「レアユニーク色」として検出し、`materials:` テンプレートに自動的に含めます。小面積の独立材料（コイル導体等）の検出漏れを防止します。
...
| `calculateMagneticField()` | flat index + `static` | B場計算（直交座標） |
| `calculateMagneticFieldPolar()` | flat index + `static` | B場計算（極座標） |
| `calculateHFieldAtActiveCells()` | LUT + `static` | H場計算（活性セル） |
| `updateMuAtActiveCells()` | LUT + `static` | μ更新（活性セル） |
| `updateMuDiffAtActiveCells()` | LUT + `static` | μ_diff更新（活性セル） |
| `updateMuDiffDistribution()` | LUT + `static` | μ_diff更新（全グリッド） |
| `interpolateMuToFullGrid()` | LUT + `static` | μ補間（全グリッド） |
| `calculateHField()` | LUT + `static` | H場計算（Picard用） |
| `updateMuDistribution()` | LUT + `static` | μ更新（Picard用） |

#### RGB→材料 LUT（`7a14595`）

非線形ソルバーのホットパス関数が毎ピクセルで `config["materials"]`（yaml-cpp Node）を走査していた問題を解消。
初期化時に `unordered_map<int, MaterialLookupEntry>`（キー: `(R<<16)|(G<<8)|B`）を構築し、O(1) マッチングに改善。
これにより yaml-cpp のスレッドセーフ制約も解消され、上記9関数の OpenMP 並列化が可能に。

#### OMP スレッド数設定（`154f289`）

YAML トップレベルの `omp:` セクションでスレッド数を指定可能。
...
- RGB→材料 LUT: `unordered_map<int, MaterialLookupEntry>` による O(1) 材料マッチング（yaml-cpp 走査を排除）
- OMP スレッド数を YAML トップレベル `omp.threads` で設定可能に（`154f289`）
- WebUI: 材料ライブラリのアクティブ状態をクッキーでキャッシュし、ページリロード時に自動復元（`23a64cb`）
...
```
afced29 WebUI: Include rare-unique colors in detect colors output
23a64cb WebUI: Auto-restore last active material library via cookie
154f289 Config: Move OMP thread setting from solver.omp_threads to omp.threads
7a14595 Perf: Add RGB-to-material LUT and parallelize hot-path functions with OpenMP
6dbb2da Docs: Add Phase 6 Newton convergence fix to v1.3.0 release notes
